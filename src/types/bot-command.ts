	...
	register(bot: Telegraf<MyContext>) {
		...
		for (const a of this.alias()) {
			bot.command(a, async ctx => {
				...
				this._$log.debug(`Command '${text}' from '${ctx.session.username || ctx.session.chatId}' processing started`);

				await this.process(ctx);

				this._$log.debug(`Command '${command}' processing finished in ${new Date().getTime() - timer.getTime()}ms`);
			})
		}
	}
